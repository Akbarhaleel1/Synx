...
const airbnb = async (url) => {
  ...
  try {
    ...
    const filteredReviews = convertToNormalDateAndExtractRating(reviews);
    console.log(filteredReviews);
    return filteredReviews;
...
const trustpilot = async (url) => {
  ...
  try {
    ...
    async function clickSort(page) {
      try {
        const sortButton = await page.waitForSelector('button[name="sort"]', { timeout: 25000, visible: true });
    ...
    async function sortRecent(page) {
      try {
        const recentButton = await page.waitForSelector('#recency', { timeout: 25000, visible: true });
        await recentButton.click();
      } catch (e) {
        console.log("Recent button not found.");
      }
    }
  ...
  try {
    ...
    const filteredReviews = reviews.map(review => {
      ...
      if (ratingMatch) {
        const rating = parseFloat(ratingMatch[1]);
        const normalizedRating = Math.round(rating / 2);
        return { ...review, date: formattedDate, rating: normalizedRating };
      }

      return { ...review, date: formattedDate, rating: null };
    });
  ...
  try {
    ...
    const reviews = await page.evaluate(() => {
      return Array.from(document.querySelectorAll('div[data-test-target="HR_CC_CARD"]')).map(element => ({
        platform: "tripadvisor",
        image: element.querySelector('img')?.src || null,
        date: element.querySelector('span[class*="reviewDate"]')?.textContent.trim() || null,
        name: element.querySelector('.ui_header_link')?.textContent.trim() || null,
        title: element.querySelector('div[class*="reviewTitle"]')?.textContent.trim() || null,
        review: element.querySelector('span[class*="reviewText"]')?.textContent.trim() || null,
        rating: element.querySelector('span[class*="ui_bubble_rating"]')?.getAttribute('class').match(/bubble_(\d+)/)[1] || null,
      }));
    });
  ...
  try {
    ...
    const reviews = await page.evaluate(() => {
      return Array.from(document.querySelectorAll('div[data-review-id]')).map(element => ({
        platform: "makemytrip",
        image: element.querySelector('img')?.src || null,
        date: element.querySelector('.review-date')?.textContent.trim() || null,
        name: element.querySelector('.user-name')?.textContent.trim() || null,
        title: element.querySelector('.review-title')?.textContent.trim() || null,
        review: element.querySelector('.review-text')?.textContent.trim() || null,
        rating: element.querySelector('.rating-value')?.textContent.trim() || null,
      }));
    });
...
// Example Usage
(async () => {
  await airbnb('YOUR_AIRBNB_URL');
  await agoda('YOUR_AGODA_URL');
  await trustpilot('YOUR_TRUSTPILOT_URL');
  await booking('YOUR_BOOKING_URL');
  await tripadvisor('YOUR_TRIPADVISOR_URL');
  await makemytrip('YOUR_MAKEMYTRIP_URL');
})();
